...
export default function MyQuestionsPage() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      if (!user) return;
      try {
        const res = await axios.get('http://localhost:5000/api/qa/user/questions', {
  ...
  if (!user) {
    return (
      <div className="card-modern p-12 text-center border border-dashed border-slate-300 bg-slate-50/50">
        ...
        <p className="text-slate-500 mt-1">You must be logged in to view your questions.</p>
        <Link href="/login" className="btn-primary mt-4 inline-flex">Go to Login</Link>
      </div>
    );
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 mb-3 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold uppercase tracking-widest">
            <i className="bi bi-person-lines-fill"></i> Personal Hub
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">My Questions</h1>
          <p className="mt-2 text-lg text-slate-600">Track and manage the community questions you've asked.</p>
        </div>
      ...
      <div className="grid gap-5">
        {loading ? (
          ...
          <div className="card-modern p-12 text-center border border-dashed border-slate-300 bg-slate-50/50">
            ...
            <p className="text-slate-500 mt-1">You haven't asked any questions in the community yet!</p>
            <Link href="/qa/ask" className="btn-primary mt-4 inline-flex">Ask your first question</Link>
          </div>
        ) : (
          questions.map(q => {
            const score = q.upvoteCount - q.downvoteCount;
            const answersCount = q.answers?.length || 0; // Note: user/questions endpoint doesn't populate answers array, wait!
            // Actually, wait, let's just show it without answers count if not populated, or we can fetch it. We will just show 0 or undefined. Let's omit answers if not available or just assume 0.
            return (
            <Link key={q._id} href={`/qa/${q._id}`} className="card-modern block p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-indigo-200 group bg-white">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                ...
                {/* Stats Section */}
                <div className="grid min-w-72 grid-cols-3 gap-3 text-center text-xs font-medium text-slate-500">
